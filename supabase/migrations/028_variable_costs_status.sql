-- ============================================================
-- Pipeline tasků: sync ukládá i nehotové tasky (status z Notionu),
-- ale do nákladů se počítají jen hotové (is_done = true).
-- Default true ⇒ existující řádky (staré importy, cesťáky) zůstávají započítané.
-- ============================================================

alter table variable_costs add column if not exists is_done boolean not null default true;
alter table variable_costs add column if not exists status text;
create index if not exists variable_costs_month_done_idx on variable_costs (month, is_done);

-- dashboard_summary: variable_costs jen s is_done = true
  ...
  with
    ...
    month_var as (
      ...
      where variable_costs.month = parsed.month
        and coalesce(variable_costs.is_done, true)
    ),
    month_extra as (
      select coalesce(sum(amount), 0)::numeric as total
